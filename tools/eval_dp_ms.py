    ...
    def eval(self, scales=[1.0]):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            with torch.no_grad():
                ...
                for scale in scales:
                    # Resize input image
                    new_size = [int(image.size(2) * scale), int(image.size(3) * scale)]
                    image_scaled = F.interpolate(image, size=new_size, mode='bilinear', align_corners=True)
                    # Run inference
                    output = model.evaluate(image_scaled)
                    # print('output.shape: ', output.shape)
                    outputs.append(output)
                # rescale the outputs to the same size
                for i in range(len(outputs)):
                    outputs[i] = F.interpolate(outputs[i], size=image.shape[2:], mode='bilinear', align_corners=True)
                # Aggregate outputs from different scales
...
if __name__ == '__main__':
    ...
    num_scale = [ 0.75, 1.0, 1.25, 1.5, 1.75]
    evaluator.eval(scales=num_scale)
